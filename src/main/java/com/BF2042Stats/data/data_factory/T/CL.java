...
public class CL implements InterfaceData {
    @Override
    public void start(GroupMessage groupMessage) {
        ...
        if (!CapacityPool.findPlayerData(name, isBD)){
            ...
            CapacityPool.addPlayerData(new PlayerData(groupMessage, name, platform, GetType.CL), isBD);
            CapacityPool.getPlayerData(name,isBD).setTime((isBD==1?24:2));
            return;
        }
        CapacityPool.getPlayerData(name, isBD).CLImg(groupMessage);
    }
}
